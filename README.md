...
> **This project was built entirely by AI** (Claude Opus 4.6 via Claude Code, and GPT-5.3 Codex). The code, architecture, and documentation were all AI-generated. While functional, it has not been through a formal security audit.
...
(human note: initially built with claude opus 4.6 in claude code. burn through only 3 full sessions of claude pro. now also trying gpt 5.3 codex on it. take this as a demo of what the current state-of-the-art code agent could do. yes, i do use this thing on my computer. i didn't check too many implementation details (i know nothing about conpty actually). i did check the content of readme, most if not all of it should be accurate.)

## How it works

1. Linux kernel's `binfmt_misc` intercepts `.exe`, `.cmd`, `.bat`, and `.ps1` execution
...
This installs the client binary, registers `binfmt_misc` handlers (by `.exe`, `.cmd`, `.bat`, and `.ps1` extension), and creates `/etc/interop.toml`. Edit the config to set your Windows host IP:
...
Once set up, run Windows binaries and script shims as if they were native:
...
When launching `.cmd`/`.bat`, the client wraps execution through `cmd.exe /d /s /c`. For `.ps1`, it wraps through `powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File`.
...
- **AI-generated code**: This entire project — including the protocol, server, client, and setup scripts — was generated by AI (Claude Opus 4.6 via Claude Code, and GPT-5.3 Codex). It has not been audited for security vulnerabilities. There may be bugs in frame parsing, path translation, handle management, or other areas that could be exploitable.
